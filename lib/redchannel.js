...
class RedChannel {
    ...
    queue_data(agent_id, command, data) {
        ...
        if (command == this.AGENT_KEYX) this.agents[agent_id].allow_keyx = true;
    ...
    implant_build(params) {
        ...
        if (!VALID_BUILD_TARGET_OS.test(os)) return { message: "invalid os value, must be supported by Go (GOOS)", error: true };
        ...
        try {
            ...
            child.on("close", (code) => {
                ...
                this.ui.success(`agent build for os: ${os}, arch: ${arch}, debug: ${debug ? "true" : "false"}, return code: ${code}`);
            });
        } catch (ex) {
