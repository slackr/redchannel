...
class RedChannel extends RedChannelObject {
    constructor(c2_message_handler) {
        ...
        // the absolute path to the app directory (set by app.js)
    ...
    send_to_proxy(agent_id, records) {
        ...
        request(options, (err, res, body) => {
            if (err) return this.ui.error(`error: failed to send data to proxy: ${str_data}`);
            if (body.length > 0) this.ui.debug(`proxy send response: ${body}`);
        });
    }
    get_from_proxy() {
        ...
        request(options, (err, res, body) => {
            if (err) return this.ui.error(`error: proxy fetch failed: ${err}`);
            ...
            if (body.length <= 5) {
                // console.log("* proxy response: " + body);
                return this.ui.error(`error: proxy response too small: ${body}`);
            }

            if (!VALID_PROXY_DATA.test(body)) return this.ui.error(`error: failed to validate incoming proxy data:\n${body}`);
    ...
    proxy_enable() {
        let message = "";
        if (this.config.proxy.enabled) message = `starting proxy checkin at interval: ${this.config.proxy.interval}ms`;
        if (!this.config.proxy.enabled) message = `stopping proxy checkin`;
    ...
    implant_build(params) {
        ...
        if (!VALID_BUILD_TARGET_ARCH.test(arch)) {
            return {
                message: "! invalid arch value, must be supported by Go (GOARCH)",
                error: true,
            };
        }
        ...
        var output_file = `${build_path}/build/agent${os === "windows" && ".exe"}`;
        var binary = "python";
        ...
        try {
            ...
            child.on("close", (code) => {
                ...
                this.log(`agent build for os: ${os}, arch: ${arch}, debug: ${debug ? "true" : "false"}, return code: ${code}`);
            });
        } catch (ex) {
            return {
                message: `! failed to launch build command: '${ex.toString()}', build command: '${binary} ${command_args.join(" ")}'`,
                error: true,
            };
        }
        ...
        return {
            message: `* building ${debug && "(debug)"} agent for os: ${os}, arch: ${arch}, binary will be available here: ${output_file} and ${binary_url}`,
            error: false,
        };
    }
    implant_log() {
