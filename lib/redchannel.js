...
class RedChannel extends RedChannelObject {
    ...
    queue_data(agent_id, command, data) {
        ...
        for (let record_num = 0; record_num < total_records; record_num++) {
            ...
            record = `${RECORD_DATA_PREFIX}:${this.pad_zero(record_num.toString(16), 4)}:${blocks.join(":")}`;
            records.push(record);

            if (total_commands > 1 && (records.length == MAX_RECORDS_PER_COMMAND - 1 || record_num == total_records - 1)) {
                record = `${RECORD_HEADER_PREFIX}:${data_id}:${this.pad_zero(command.toString(16), 2)}${this.pad_zero(padded_bytes.toString(16), 2)}:${this.pad_zero(total_records.toString(16), 4)}:0000:0000:0000:0001`;
