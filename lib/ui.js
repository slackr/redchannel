...
class RedChannelUI extends RedChannelObject {
    constructor($rc_obj, $c_obj) {
        super("ui");

        this.redchannel = $rc_obj;
        this.crypto = $c_obj;

        this.debug = $rc_obj.config.debug;
    ...
    show_agents() {
        this.msg(chalk.gray('* agents:'));

        var rows = []
        Object.keys(this.redchannel.agents).forEach((a) => {
            var ao = this.redchannel.agents[a];
            var agent_secret = ao.secret != null ? ao.secret.toString('hex') : "n/a";
            rows.push([
                chalk.blue(ao.ident),
                ao.ip,
                chalk.greenBright(agent_secret),
                new Date(ao.lastseen * 1000).toLocaleString()
            ]);
        });

        this.display_table(['id', 'src', 'secret', 'lastseen'], rows);
    }

    show_help(help_module) {
        this.msg(chalk.gray("* " + help_module + " commands:"));

        var rows = []
        Object.keys(this.redchannel.commands[help_module]).forEach((cmd) => {
            rows.push([
                chalk.yellow(cmd),
                chalk.red((this.redchannel.commands[help_module][cmd]['params']).join(' ')),
                chalk.yellow(this.redchannel.commands[help_module][cmd]['desc'])
            ]);
        });
        if (this.redchannel.using_module) {
            Object.keys(this.redchannel.commands.module_common).forEach((cmd) => {
                rows.push([
                    chalk.yellow(cmd),
                    chalk.red((this.redchannel.commands.module_common[cmd]['params']).join(' ')),
                    chalk.yellow(this.redchannel.commands.module_common[cmd]['desc'])
                ]);
            });
        }
    ...
    display_table(columns, rows) {
        ...
        this.msg(chalk.gray(table.toString()));
    }
        ...
        if (this.redchannel.agent.interact) {
            switch (command) {
                case "debug":
                    ...
                    this.redchannel.config.debug = this.debug;
                    this.msg(chalk.yellowBright("! debug " + (this.debug ? "enabled" : "disabled")));
                    break;
                case "back":
                ...
                case "sysinfo":
                    if (!this.redchannel.agent.interact.secret) {
                        this.msg(chalk.redBright("! cannot send sysinfo to " + chalk.blue(this.redchannel.agent.interact.ident) + ", start 'keyx' first"));
                        break;
                    }
                    ...
                    this.msg(chalk.yellow("* requesting sysinfo from " + chalk.blue(this.redchannel.agent.interact.ident)));
                ...
                case "interact":
                    ...
                    if (typeof with_who == "undefined" || with_who.length == 0) {
                        this.msg(chalk.redBright("! please specify an agent id, see 'agents'"));
                        break;
                    }

                    this.redchannel.agent.interact = this.redchannel.get_agent(with_who);
                    if (!this.redchannel.agent.interact) {
                        this.msg(chalk.redBright("! agent '" + chalk.blue((this.redchannel.agent.interact ? with_who : "")) + "' not found"));
                    } else {
                        this.msg(chalk.yellowBright("* interacting with " + chalk.blue(this.redchannel.agent.interact.ident)));
                        this.reset_prompt();
                    }
                    break;
                case "agents":
                ...
                case "shutdown":
                    if (!this.redchannel.agent.interact.secret) {
                        this.msg(chalk.redBright("! cannot send shutdown to " + chalk.blue(this.redchannel.agent.interact.ident) + ", start 'keyx' first"));
                        break;
                    }

                    var shutdown_who = param.shift();
                    if (shutdown_who !== this.redchannel.agent.interact.ident) {
                        this.msg(chalk.yellow("* please confirm shutdown by entering the agent id, see 'help'"));
                        break;
                    }
                    ...
                    this.msg(chalk.yellow("* sending shutdown command to " + chalk.blue(this.redchannel.agent.interact.ident)));
                    this.redchannel.command_shutdown(tag);
                    break;
                case "shell":
                case "exec_cmd":
                    if (!this.redchannel.agent.interact.secret) {
                        this.msg(chalk.redBright("! cannot send command to " + chalk.blue(this.redchannel.agent.interact.ident) + ", start 'keyx' first"));
                        break;
                    }

                    var cmd = param.join(' ');
                    if (cmd.length == 0) {
                        this.msg(chalk.redBright("! command failed, insufficient parameters, see 'help'"));
                        break;
                    }
                    ...
                    this.msg(chalk.yellowBright("* sending shell command to " + chalk.blue(this.redchannel.agent.interact.ident) + ""));
                    this.redchannel.command_shell(payload);
                    break;
                case "set":
                    if (!this.redchannel.agent.interact.secret) {
                        this.msg(chalk.redBright("! cannot send config to " + chalk.blue(this.redchannel.agent.interact.ident) + ", start 'keyx' first"));
                        break;
                    }
                    ...
                    if (!Object.keys(agent_settings).includes(setting)) {
                        this.msg(chalk.redBright("! invalid config setting, see 'help'"));
                        break;
                    }

                    var value = param.join(' ');
                    if (value.length === 0) {
                        this.msg(chalk.redBright("! please specify config setting value, see 'help'"));
                        break;
                    }

                    // TOOD: regex validate value
                    ...
                    this.msg(chalk.greenBright("* setting '" + config_name_map[setting] + "' to value '" + value + "' on agent: " + this.redchannel.agent.interact.ident));
                ...
                case "keyx":
                    ...
                    this.msg(chalk.yellowBright("* keyx started with agent " + chalk.blue(this.redchannel.agent.interact.ident)));
                    this.redchannel.command_keyx(this.crypto.export_pubkey('uncompressed'), this.redchannel.agent.interact.ident);
                    break;
                case "msg":
                    if (!this.redchannel.agent.interact.secret) {
                        this.msg(chalk.redBright("! cannot send msg to " + chalk.blue(this.redchannel.agent.interact.ident) + ", start 'keyx' first"));
                        break;
                    }
                    ...
                    this.msg(chalk.yellowBright("* sending message to " + chalk.blue(this.redchannel.agent.interact.ident) + ""));
                    this.redchannel.queue_data(this.redchannel.agent.interact.ident, this.redchannel.AGENT_MSG, payload);
                    break;
                default:
                    this.msg(chalk.red("! invalid command: " + command + ", see 'help'"));
                    break;
            }
            ...
            switch (command) {
                ...
                case "reset":
                    ...
                    this.msg(chalk.gray("* module configuration reset, see 'config'"));
                    break;
                case "ls":
                case "info":
                case "config":
                    this.msg(chalk.gray("* module config:"));
                    this.msg(chalk.gray(JSON.stringify(this.redchannel.config[n], null, "  ")));
                    break;
                case "help":
                    this.show_help(n);
                    break;
                ...
                case "set":
                    ...
                    if (setting_type === "undefined") {
                        this.msg(chalk.redBright("! unknown module setting '" + setting + "', see 'help'"));
                        break;
                    }

                    var p = param.join(' ');
                    if (p.length === 0) {
                        this.msg(chalk.redBright("! please specify setting value, see 'help'"));
                        break;
                    }

                    // get the help object for the command (for set, its 'set property')
                    var command_help = this.redchannel.commands[n][command + ' ' + setting];
                    if (typeof command_help.validate_regex == "object"
                        && typeof command_help.validate_regex.test == "function"
                        && !command_help.validate_regex.test(p)) {
                        this.msg(chalk.redBright("! invalid setting value, see 'help'"));
                        break;
                    }
                    switch (setting_type) {
                        case "number":
                    ...
                    this.msg(chalk.gray("* '" + setting + "' set to '" + this.redchannel.config[n][setting] + "'"))
                    break;
                default:
                    ...
                    if (typeof this.redchannel.modules[n].actions[command] === "function") {
                        ...
                        var ret = this.redchannel.modules[n].actions[command].bind(this.redchannel)(param);
                        if (ret.error) {
                            this.msg(chalk.redBright(ret.message));
                        } else if (ret.message.length > 0) {
                            this.msg(chalk.greenBright(ret.message));
                        }
                        break;
                    }
                    this.msg(chalk.redBright("! invalid command: " + command + ", see 'help'"));
                    break;
            }
        } else { // main menu
            switch (command) {
                ...
                case "interact":
                    ...
                    this.redchannel.agent.interact = this.redchannel.get_agent(with_who);
                    if (!this.redchannel.agent.interact) {
                        this.msg(chalk.redBright("! agent " + chalk.blue(with_who) + " not found"));
                    } else {
                        this.msg(chalk.gray("* interacting with " + chalk.blue(this.redchannel.agent.interact.ident) + ""));
                    }
                    break;
                case "keyx":
                    if (!this.crypto.key) {
                    ...
                    this.msg(chalk.yellowBright("* keyx started with all agents"));
                ...
                case "kill":
                    ...
                    if (!agent_to_kill) {
                        this.msg(chalk.redBright("! agent " + chalk.blue(kill_who) + " not found"));
                    } else {
                        this.msg(chalk.yellow("* killing " + chalk.blue(agent_to_kill.ident) + ", agent may reconnect"));
                ...
                case "use":
                    ...
                    if (module_name.length == 0 || typeof this.redchannel.modules[module_name] === "undefined") {
                        this.msg(chalk.redBright("! unknown module: '" + module_name + "', see 'help'"));
                        break;
                    }
